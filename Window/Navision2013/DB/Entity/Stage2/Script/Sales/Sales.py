    ...
    for dbe in config["DbEntities"]:
        if dbe['ActiveInactive']=='true' and  dbe['Location']==DBEntity:
            ...
            try:
                ...
                GLEntry_Sales.write.option("maxRecordsPerFile", 10000).format("parquet").mode("overwrite").option("overwriteSchema", "true").save(STAGE2_PATH+"/"+"Sales/SalesGLEntry")
                GLEntry_DE = GLEntry_DE.join(DSE,"DimensionSetID",'left')
                GLEntry_DE.cache()
                print(GLEntry_DE.count())
                GLEntry_DE.write.option("maxRecordsPerFile", 10000).format("parquet").mode("overwrite").option("overwriteSchema", "true").save(STAGE2_PATH+"/"+"Sales/ManualCOGS")   
                finalDF.cache()
                print(finalDF.count())
                finalDF=finalDF.drop('SystemEntry','Item_No')
                finalDF.write.option("maxRecordsPerFile", 10000).format("parquet").mode("overwrite").option("overwriteSchema", "true").save(STAGE2_PATH+"/"+"Sales/Sales")
